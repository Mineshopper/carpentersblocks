...
public abstract class AbstractAttribute<K> {
    ...
    public void writeToNBT(CompoundNBT nbt) {
    	nbt.putInt(TAG_ATTR_LOCATION, _location.ordinal());
    	nbt.putInt(TAG_ATTR_TYPE, _type.ordinal());
    	nbt.putString(TAG_ATTR_IDENT, getModelIdentifier());
    	writeModelToNBT(nbt);
    }
    
    public void readFromNBT(CompoundNBT nbt) {
    	_location = EnumAttributeLocation.valueOf(nbt.getInt(TAG_ATTR_LOCATION) & 255);
    	_type = EnumAttributeType.valueOf(nbt.getInt(TAG_ATTR_TYPE) & 255);
    	readModelFromNBT(nbt);
    }
    
    public abstract String getModelIdentifier();
    
    public abstract void writeModelToNBT(CompoundNBT nbt);
    
    public abstract void readModelFromNBT(CompoundNBT nbt);
