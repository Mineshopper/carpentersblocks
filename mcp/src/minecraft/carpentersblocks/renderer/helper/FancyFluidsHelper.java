...
public class FancyFluidsHelper {
        ...
        if (itemStack != null) {
            ...
            if (block.getRenderBlockPass() == MinecraftForgeClient.getRenderPass())
            {
                if (!block.hasTileEntity(metadata) && metadata == 0)
                {
                    ...
                    double fluidHeight = (block instanceof BlockFluid ? 1.0D - 1.0F / 9.0F : 0.875F) - 0.0010000000474974513D;
    ...
    public static ItemStack getFluidBlock(IBlockAccess world, int x, int y, int z)
    {
        ...
        for (int idx = 0; idx < offsetXZ.length; ++idx) {
            ...
            Block block = Block.blocksList[world.getBlockId(x + offsetXZ[idx][0], y, z + offsetXZ[idx][1])];

            if (block != null) {
                if (block instanceof BlockFluid || block instanceof IFluidBlock) {
                    if (idx < 4) {
                        if (!world.isBlockSolidOnSide(x, y, z, route[idx][0][0], false)) {
                        ...
                        for (int routeIdx = 0; routeIdx < 2; ++routeIdx) {
                            if (!world.isBlockSolidOnSide(x, y, z, route[idx][routeIdx][0], false)) {
                                int[] bridgeXZ = { x + route[idx][routeIdx][0].offsetX, z + route[idx][routeIdx][0].offsetZ };
                                if (!world.isBlockSolidOnSide(bridgeXZ[0], y, bridgeXZ[1], route[idx][routeIdx][1], false) && !world.isBlockSolidOnSide(bridgeXZ[0], y, bridgeXZ[1], route[idx][routeIdx][2], false)) {
                                    return new ItemStack(block, world.getBlockMetadata(x + offsetXZ[idx][0], y, z + offsetXZ[idx][1]));
                                }
                            }
                        }
