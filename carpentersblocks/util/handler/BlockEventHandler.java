...
public class BlockEventHandler {
	...
	public void playerInteractEvent(PlayerInteractEvent event) {
		int blockID = event.entity.worldObj.getBlockId(	event.x,
														event.y,
														event.z);
		...
		if (blockID > 0 && Block.blocksList[blockID] instanceof BlockBase) {
			...
			if (event.action.equals(PlayerInteractEvent.Action.LEFT_CLICK_BLOCK)) {
				...
				/*
				 * Call onBlockActivated() if player is holding the Carpenter's
				 * Hammer and sneaking.
				 */
				if (itemStack != null
					&& itemStack.itemID == ItemHandler.itemCarpentersHammerID) block.onBlockActivated(	event.entity.worldObj,
																										event.x,
																										event.y,
																										event.z,
																										event.entityPlayer,
