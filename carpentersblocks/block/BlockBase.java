...
public class BlockBase extends BlockContainer {
    ...
    /**
     * Returns whether block is covered.
     * Used to decide which block to use for method calls.
     */
    protected boolean isCovered(IBlockAccess world, int x, int y, int z)
    {
        TEBase TE = (TEBase) world.getBlockTileEntity(x, y, z);

        return BlockProperties.hasCover(TE, 6);
    }
    ...
    protected boolean isValid(IBlockAccess world, int x, int y, int z)
    {
        ...
        return block != null && block.equals(this) || TE != null;
    }
        ...
        if (world.isRemote)
        {
            ...
            if (itemStack != null) {
                if (itemStack.getItem() instanceof ItemBlock) {
                    if (Block.blocksList[itemStack.itemID] instanceof BlockBase) {
                        return false;
                    }
                } else {
