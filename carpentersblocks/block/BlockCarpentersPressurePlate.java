...
public class BlockCarpentersPressurePlate extends BlockBase
{
    ...
    protected boolean onHammerLeftClick(TECarpentersBlock TE, EntityPlayer entityPlayer)
    {
    	...
    	if (!TE.worldObj.isRemote) {
	    	...
	    	TE.worldObj.notifyBlocksOfNeighborChange(TE.xCoord, TE.yCoord - 1, TE.zCoord, this.blockID);
    	} else {
	    	switch (polarity) {
	...
	public void setBlockBoundsBasedOnState(IBlockAccess world, int x, int y, int z)
    {
        ...
        this.setBlockBounds(0.0625F, 0.0F, 0.0625F, 1.0F - 0.0625F, isDepressed(TE) ? 0.03125F : 0.0625F, 1.0F - 0.0625F);
    }
    ...
    public void updateTick(World world, int x, int y, int z, Random random)
    {
    	if (!world.isRemote)
    	{
    		...
    		List entityList = world.getEntitiesWithinAABB(Entity.class, this.getSensitiveAABB(x, y, z));

    		if (!entityList.isEmpty()) {
    			for (int count = 0; count < entityList.size(); ++count)
    				this.setStateIfMobCollidesWithPlate(TE, (Entity)entityList.get(count), world, x, y, z);
    		} else {
    			this.setStateIfMobCollidesWithPlate(TE, (Entity)null, world, x, y, z);
    		}
    	}
    }
    ...
    private void setStateIfMobCollidesWithPlate(TECarpentersBlock TE, Entity entity, World world, int x, int y, int z)
    {
        ...
        if (this.shouldTrigger(TE, entity, world, x, y, z))
        {
	...
	public int isProvidingStrongPower(IBlockAccess world, int x, int y, int z, int side)
    {
        ...
        return side == 1 ? this.getPowerSupply(TE, data) : 0;
    }
