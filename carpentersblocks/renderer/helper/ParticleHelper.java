		...
		for (int posX = 0; posX < factor; ++posX)
		{
			for (int posY = 0; posY < factor; ++posY)
			{
				for (int posZ = 0; posZ < factor; ++posZ)
				{
					...
					EntityDiggingFX particle = new EntityDiggingFX(world, dirX, dirY, dirZ, dirX - x - 0.5D, dirY - y - 0.5D, dirZ - z - 0.5D, block, -1, metadata, FMLClientHandler.instance().getClient().renderEngine);
