...
public class ParticleHelper {
		...
		for (int posX = 0; posX < factor; ++posX)
		{
			for (int posY = 0; posY < factor; ++posY)
			{
				for (int posZ = 0; posZ < factor; ++posZ)
				{
					...
					EntityDiggingFX particle = new EntityDiggingFX(world, dirX, dirY, dirZ, dirX - (double) x - 0.5D, dirY - (double) y - 0.5D, dirZ - (double) z - 0.5D, block, -1, metadata, FMLClientHandler.instance().getClient().renderEngine);
	...
	{
		EntityDiggingFX particle = new EntityDiggingFX(TE.worldObj, x, y, z, 0.0D, 0.0D, 0.0D, block, target.sideHit, metadata, FMLClientHandler.instance().getClient().renderEngine);
	...
	public static Block getParticleBlockFromOverlay(TEBase TE, int coverSide, Block block)
	{
		if (BlockProperties.hasOverlay(TE, coverSide))
		{
			...
			case OverlayHandler.OVERLAY_HAY:
				// Can't actually use hay override in MC 1.5+, so do nothing
				break;
